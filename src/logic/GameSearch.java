...
public class GameSearch {         // algorithm Minimax
    ...
    public static MoveList findAllValidMoves(final Board board, int color) {
        ...
        for (int i = 1; i < 33; i++) {
            c = new Coordinate(i);
            if ((board.getChecker(c) != null) && (board.getChecker(c).getColor() == color)) {
                if (jumpExist) {
    ...
    public static Board minimaxAB(Board board, int recursion, int player, Board alpha, Board beta) {
        if (recursion > 0) {
            ...
            if (player == CheckerPosition.BLACK) {   // Black - min node.
                ...
                while (iterator.hasNext()) {
                    Board nextBoard = minimaxAB(executeMove(iterator.next(), board),
                            recursion - 1, opponent(player), alpha, beta);
    ...
    public static boolean validWhiteMove(Coordinate from, Coordinate to, Board board) {
        return validUpMove(from, to, board) && (board.getChecker(from).getColor() == CheckerPosition.WHITE);
    }

    public static boolean validBlackMove(Coordinate from, Coordinate to, Board board) {
        return validDownMove(from, to, board) && (board.getChecker(from).getColor() == CheckerPosition.BLACK);
    }
    ...
    private static boolean validUpMove(Coordinate from, Coordinate to, Board board) {
        return validCoordinate(from) && validCoordinate(to)
                ...
                && (from.row() - to.row() == 1) && ((from.upLeftMove().equals(to))
    ...
    private static boolean validDownMove(Coordinate from, Coordinate to, Board board) {
        return validCoordinate(from) && validCoordinate(to)
                && board.vacantCoordinate(to) && !board.vacantCoordinate(from)
                && (to.row() - from.row() == 1) && ((from.downLeftMove().equals(to))
    ...
    private static boolean validUpJump(Coordinate from, Coordinate to, int color, Board board) {
        boolean basis = validCoordinate(from) && validCoordinate(to)
                && board.vacantCoordinate(to) && !board.vacantCoordinate(from)
                && (board.getChecker(from).getColor() == color) && (from.row() - to.row() == 2);
    ...
    private static boolean validDownJump(Coordinate from, Coordinate to, int color, Board board) {
        boolean basis = validCoordinate(from) && validCoordinate(to)
                && board.vacantCoordinate(to) && !board.vacantCoordinate(from)
                && (board.getChecker(from).getColor() == color) && (to.row() - from.row() == 2);
